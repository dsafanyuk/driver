...
#define MAX_PENDING_REQUESTS 20 /* Maximum pending requests        */
                                /* allowed                         */
    ...
    while (true)
    {
        ...
        if (p_pending_request_list->p_next_request->p_next_request != NULL)
        {
            ...
            if (disk_on == false)
            {
                ...
                /* Loop with empty body, wait until the disk motor is up  */
                /* to speed                                               */
            ...
            if (error_code == 0)
            {
                ...
                if (disk_heads != cylinder)
                {
                    ...
                    disk_heads = disk_drive(SEEK_TO_CYLINDER, cylinder, 0, 0, 0);

                    /* Check if the disk heads land correctly              */
                    while (disk_heads != cylinder)
                    {
                        /* Loop with empty body, wait until the disk heads  */
                        /* land on cylinder zero                            */
                        ...
                        {
                            disk_heads =
                                disk_drive(SEEK_TO_CYLINDER, cylinder, 0, 0, 0);
                        }
                    }
                }

                /* Process the request if DMA sets up correctly           */
                if (disk_drive(DMA_SETUP, sector, track, BYTES_PER_SECTOR *
                               SECTORS_PER_BLOCK, 
